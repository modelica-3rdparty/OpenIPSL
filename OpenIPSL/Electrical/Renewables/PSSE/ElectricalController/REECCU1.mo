...
model REECCU1
  ...
  extends
    OpenIPSL.Electrical.Renewables.PSSE.ElectricalController.BaseClasses.BaseREECC(       Iqcmd(start = Iq0), Ipcmd(start = Ip0));
  ...
  OpenIPSL.Electrical.Renewables.PSSE.ElectricalController.BaseClasses.SOC_logic
                                            sOC_logic(SOCmin=SOCmin, SOCmax=
        SOCmax)      annotation (Placement(transformation(extent={{-30,-244},{-10,
            -224}})));
  Modelica.Blocks.Sources.BooleanConstant PfFlag_logic(k=pfflag)
    annotation (Placement(transformation(extent={{-204,44},{-184,64}})));
...
equation
  ...
  connect(SOC, add1.u2)
    annotation (Line(points={{-320,-240},{-106,-240}}, color={0,0,127}));
  ...
  connect(PfFlag_logic.y,PfFlag. u2) annotation (Line(points={{-183,54},{-176,54},
          {-176,88},{-208,88},{-208,146},{-200,146}},
                                  color={255,0,255}));
  ...
  connect(add6.y,variableLimiter1. u)
    annotation (Line(points={{147,128},{158,128}}, color={0,0,127}));
  connect(IQMAX_.y,variableLimiter1. limit1) annotation (Line(points={{159,154},
          {152,154},{152,136},{158,136}}, color={0,0,127}));
  connect(variableLimiter1.y,QFlag. u1)
    annotation (Line(points={{181,128},{196,128},{196,102}},color={0,0,127}));
